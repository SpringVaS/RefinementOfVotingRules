...
definition plurality_ref :: "'a::{default, heap, hashable} Electoral_Module_Ref" where
  "plurality_ref A pl \<equiv> do {
   scores \<leftarrow> (pre_compute_scores plurality_score_ref A pl);
   max_eliminator_ref scores A pl
}"

lemma plurality_score_correct:
  ...
  assumes 
    ...
    profp: "profile A pr"
  shows "\<forall> a \<in> A. plurality_score_ref a A pl \<le> SPEC (\<lambda> sc. sc = (plurality_score a A pr))"
  unfolding plurality_score_ref_def plurality_score.simps
proof safe
  fix a :: 'a
  from prel profp have profl: "profile_l A pl" using profile_ref by auto 
  show "wc_fold pl a \<le> SPEC (\<lambda>sc. sc = win_count pr a) "
    by (refine_vcg fina prel profl nemp_A wc_fold_correct)
qed
...
definition plurality_ref_opt :: "'a::{default, heap, hashable} Electoral_Module_Ref" where
  "plurality_ref_opt A pl \<equiv> do {
   scores \<leftarrow> (pre_compute_plurality_scores A pl);
   max_eliminator_ref scores A pl
}"
...
lemma plurality_map_correct:
  ...
  shows "(pre_compute_plurality_scores A pl) \<le> SPEC (\<lambda> map. map =(scores_map plurality_score A pr))"
  unfolding pre_compute_plurality_scores_def scores_map_def plurality_score.simps
...
next
  ...
  from this have "set xa = A"
    using profl  unfolding profile_l_def  lin_order_equiv_list_of_alts
    by (metis in_set_conv_decomp in_set_conv_nth)
  from this nempxa have innit: "xa ! 0 \<in> A" by auto
  assume somev: 
    "\<forall>a\<in>A. sigma a =
              Some
               (fold
                 (\<lambda>x ac. if x \<noteq> [] \<and> x ! 0 = a then ac + 1 else ac)
                 l1 0)"
  assume " \<forall>a. a \<notin> A \<longrightarrow> sigma a = None"
  from this somev innit show "\<exists>y. sigma (xa ! 0) = Some y"
    by simp
next
  fix xa :: 'a
  assume nemp_A: "A \<noteq> {}"
  ...
  from prel profl have "(pl, pr) \<in> profile_on_A_rel A"
    by (simp add: in_br_conv list_rel_eq_listrel listrel_iff_nth profile_l_def relAPP_def
        relcomp.intros)
  from this wclmap have wcmap: 
    "(sigma) = (\<lambda>a . Some (win_count pr a))|`A"
    using  win_count_l_correct[THEN fun_relD,THEN fun_relD, where x1 = pl and x'1 = pr and A2 = A]
    by fastforce
  from this show
    "sigma = (\<lambda>a. Some (card {i. i < length pr \<and> above (pr ! i) a
         = {a}})) |` A" by simp
qed
...
proof (intro frefI nres_relI,  clarsimp simp del:  plurality.simps plurality_score.simps,
     unfold plurality_ref_def plurality.simps
  ...
  note mec= max_eliminator_ref_correct_default[where A = A and efn_ref = plurality_score_ref and
       efn = plurality_score and pl = pl and pr = pr]
  have " \<forall>a\<in>A. plurality_score_ref a A pl \<le> SPEC (\<lambda>score. score = plurality_score a A pr)"
  proof (cases "A = {}")
    case True
    then show ?thesis
      by fastforce 
  next
    case False
    then show ?thesis using fina prel profp plurality_score_correct by blast
  qed
  thus " pre_compute_scores plurality_score_ref A pl \<bind> (\<lambda>scores. max_eliminator_ref scores A pl)
       \<le> SPEC (\<lambda>x. x = max_eliminator plurality_score A pr)"
    using mec fina prel
    by blast
qed

lemma plurality_ref_opt_correct:
  shows "(uncurry plurality_ref_opt, uncurry (RETURN oo plurality)) \<in> 
  ([\<lambda> (A, pl). finite_profile A
            pl]\<^sub>f (\<langle>Id\<rangle>set_rel \<times>\<^sub>r profile_rel)
   \<rightarrow> \<langle>\<langle>Id\<rangle>set_rel \<times>\<^sub>r \<langle>Id\<rangle>set_rel \<times>\<^sub>r \<langle>Id\<rangle>set_rel\<rangle>nres_rel)"
proof (intro frefI nres_relI,  clarsimp simp del:  plurality.simps plurality_score.simps,
     unfold plurality_ref_opt_def plurality.simps
     RETURN_SPEC_conv, rename_tac A pl pr)
  fix A :: "'a::{default, heap, hashable} set"
  fix pr :: "'a Profile"
  fix pl :: "'a Profile_List"
  ...
  show "pre_compute_plurality_scores A pl \<bind> (\<lambda>scores. max_eliminator_ref scores A pl)
       \<le> SPEC (\<lambda>x. x = max_eliminator plurality_score A pr)"
...
sepref_definition plurality_elim_sep is
  ...
  unfolding plurality_ref_def  max_eliminator_ref_def plurality_score_ref_def
...
sepref_definition plurality_elim_sep_opt is
  "uncurry plurality_ref_opt":: 
   ...
   \<rightarrow>\<^sub>a (result_impl_assn id_assn)"
  unfolding plurality_ref_opt_def  max_eliminator_ref_def plurality_score_ref_def
    less_eliminator_ref_def  elimination_module_ref_def[abs_def] eliminate_def[abs_def]
    scoremax_def[abs_def] op_set_is_empty_def[symmetric]
   pre_compute_plurality_scores_def init_map_def op_set_is_empty_def[symmetric] 
     hm.fold_custom_empty 
  apply (rewrite in "RETURN (_, _, rewrite_HOLE)" hs.fold_custom_empty)
  apply (rewrite in "RETURN (_, rewrite_HOLE, _)" hs.fold_custom_empty)+
  apply (rewrite in "RETURN (rewrite_HOLE, _, _)" hs.fold_custom_empty)+
...
lemma plurality_elim_sep_correct:
  shows "(uncurry plurality_elim_sep_opt, uncurry (RETURN \<circ>\<circ> plurality))
