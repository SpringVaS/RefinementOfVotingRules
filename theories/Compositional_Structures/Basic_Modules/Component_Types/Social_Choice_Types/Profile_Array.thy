...
value "list_of_array (array_of_list [1::nat,2])"
...
definition is_less_pref_array ::"'a \<Rightarrow> 'a Preference_Array \<Rightarrow> 'a \<Rightarrow> bool nres" where
  "is_less_pref_array x ballot y \<equiv> do {
    (i, rank) \<leftarrow> WHILET (\<lambda>(i, rank). (i < (array_length ballot))) 
      (\<lambda>(i, rank). do {
      let c = ballot[[i]];
...
definition "rank_array_invariant ballot_a a \<equiv> \<lambda> (i, rank). 
  \<forall>idx < i. ballot_a[[idx]] \<noteq> a \<or> rank = i"

definition rank_array_mon :: "'a Preference_Array \<Rightarrow> 'a \<Rightarrow> nat nres" where
  "rank_array_mon ballot_a a \<equiv> do {
    (i, rank) \<leftarrow> WHILET (\<lambda>(i, rank). (i < (array_length ballot_a) \<and> rank = 0)) 
      (\<lambda>(i, rank). do {
      let rank = (if (ballot_a[[i]] = a) then (i + 1) else 0);
      let i = i + 1;
      RETURN (i, rank)
    })(0,0);
    RETURN rank
...
lemma rank_array_mon_correct: assumes prof: "well_formed_prefa a" 
  shows "rank_array_mon ballot_a a \<le> SPEC (\<lambda> r. r = rank (pl_\<alpha> (list_of_array ballot_a)) a)"
  unfolding rank_array_mon_def rank_l.simps
  apply (intro WHILET_rule[where I="(rank_array_invariant ballot_a a)" and R="measure (\<lambda>(i,_). (array_length ballot_a) - i)"] refine_vcg)
  unfolding rank_array_invariant_def
  apply auto
proof -
  fix i
  assume nir: "\<not> i < array_length ballot_a"
  from nir prof have "\<forall>idx < array_length ballot_a. ballot_a[[idx]] \<noteq> a " 
    unfolding well_formed_prefa_def 
