...
proof (unfold pl_\<alpha>_def, safe, clarsimp_all, safe)
  ...
  from tupler lor have aA: "a \<in> A"
    unfolding linear_order_on_def partial_order_on_def preorder_on_def refl_on_def
    by blast
  from lol have "set l = A"
    by (simp add: lin_order_equiv_list_of_alts)
...
next
  ...
  from tupler lor have aA: "b \<in> A"
    unfolding linear_order_on_def partial_order_on_def preorder_on_def refl_on_def
    by blast
  from lol have "set l = A" unfolding lin_order_equiv_list_of_alts by simp
  from aA this show "b \<in> set l" by simp
next
  fix a b :: 'a
  assume tupler: "(a, b) \<in> r"
  from lol have Al: "set l = A" unfolding lin_order_equiv_list_of_alts by simp
  from tupler lor have aA: "a \<in> A"
    unfolding linear_order_on_def partial_order_on_def preorder_on_def refl_on_def
    by blast
  ...
  from tupler lor have "{ba. (b, ba) \<in> r} \<subseteq> {b. (a, b) \<in> r}"
    unfolding rank.simps above_def linear_order_on_def partial_order_on_def preorder_on_def refl_on_def
              trans_def antisym_def total_on_def Relation.trans_def
    by blast
...
next
  ...
  proof (unfold rank.simps above_def)
    ...
    by (metis Al Preference_Relation.above_trans above_def al bl card_mono fina lor order_antisym
        rank.simps rank_unique)
  qed
qed

lemma new_try_complete_refine:
  fixes A :: "'a set"
...
proof (-)
  from assms
  obtain ballo :: "'a Preference_List" where ballodef: "(\<forall> x \<in> A. set (above_l ballo x) = above bar x)"
...
assumes fina: "finite A"
  and lo: "linear_order_on A bar"
  shows "\<exists> bal :: 'a Preference_List. (bal, bar) \<in> ballot_rel"
  unfolding in_br_conv pl_\<alpha>_def 
  using assms
proof (induction)
  case empty
  then have "bar = {}"
  ...
  then show ?case
    apply (rule_tac x = "[]" in exI) unfolding ballot_rel_def
...
next
  ...
  obtain constabs where ca: "constabs = baro \<union> {(a, b) \<in> bar. a \<in> F \<and> b = x} \<union> {(x,x)} \<union> {(a, b) \<in> bar. a = x \<and> b \<noteq> x}"
    by auto

  have bardef: "constabs = bar" unfolding ca defbaro limit.simps using ih(4)
    unfolding linear_order_on_def partial_order_on_def preorder_on_def refl_on_def
    by auto
  ...
  have lonb: "linear_order_on (insert x F) (pl_\<alpha> newbal)"
    using seteqnb[symmetric] unfolding lin_order_equiv_list_of_alts[symmetric]
    by (simp add: lin_ord_equiv) 
  have barolim: "baro = limit F (pl_\<alpha> newbal)"
    using limit_equiv[where l=newbal and A = F] disnb
    unfolding  balolim[symmetric] absto[symmetric] by simp
  ...
  have lobs: "linear_order_on_l ({x} \<union> F) newbal"
    using lin_ord_equiv lonb by auto
  have lolim: "linear_order_on F (limit F (pl_\<alpha> newbal))"
    using barolim lobaro by blast
  from rankxgt0 rankib have newbal_x: "((take ((rank bar x) - 1) (balo)) @ [x])!((rank bar x)-1) = x"
  ...
  from fprefs this have rankxabs: "rank (pl_\<alpha> newbal) x = (rank bar x)" 
    using rank_equiv disnb 
    unfolding distinct_def
    by metis
  then have "\<forall> b \<in> F. (x \<preceq>\<^sub>(pl_\<alpha> newbal) b) = (x \<preceq>\<^sub>(bar) b)"
    using lonb
    unfolding is_less_preferred_than.simps rank.simps above_def
  show ?case unfolding ballot_rel_def
    in_br_conv 
    apply (rule_tac x = newbal in exI)
  proof (auto)
    show "\<And>a b. (a, b) \<in> bar \<Longrightarrow> (a, b) \<in> pl_\<alpha> newbal"
      using  fprefs ih(1) ih(2) ih(4) bardef[symmetric]
      lonb
      unfolding is_less_preferred_than.simps ca
    
  have "\<forall>a\<in> F. rank (limit F (pl_\<alpha> newbal)) a = rank (limit F bar) a"
    using rankxabs ca onFeq ih.hyps(1) rank_unique[where A = "F" and r = "limit F (pl_\<alpha> newbal)"]
    by auto

  then have "\<forall>a\<in> insert x F. rank (pl_\<alpha> newbal) a = rank bar a"
    using rankxabs limit_presv_above_2 limit_presv_above finxF lonb rank_unique[where A = "insert x F" and r = "(pl_\<alpha> newbal)"]
    unfolding rank.simps

  from this onFeq fdx cardxinc above_presv_limit limit_presv_lin_ord
limit_presv_prefs_2[where A = "F" and r = "(pl_\<alpha> newbal)"] limit_presv_prefs_2[where A = "F" and r = "bar"] have "\<forall>a\<in> F. rank (pl_\<alpha> newbal) a = rank bar a"
    unfolding rank.simps is_less_preferred_than.simps above_def
    
  from ih(4) finxF posxl have "bar = (pl_\<alpha> newbal)"
    using 
    disnb lobs rankeq_imp_ref[where A = "{x} \<union> F" and r = bar and l = newbal]
    sorry
  from this show ?case
    apply (rule_tac x = newbal in exI)
    unfolding ballot_rel_def distinct_def in_br_conv using disnb by auto
qed
